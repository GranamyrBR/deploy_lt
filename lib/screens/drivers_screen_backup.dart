...
class _DriversScreenState extends ConsumerState<DriversScreen> {
  ...
  void _showAddDriverDialog() {
    ...
    final isLoadingCars = ref.read(driverProvider).isLoading && (car == null || car.id == 0);
  ...
  List<Driver> _getFilteredDrivers(List<Driver> driver) {
    if (_searchTerm.isEmpty) {
      return driver;
    }

    return driver.where((driver) {
      final nameMatch = driver.name.toLowerCase().contains(_searchTerm);
      final emailMatch = driver.email?.toLowerCase().contains(_searchTerm) ?? false;
      final phoneMatch = driver.phone?.contains(_searchTerm) ?? false;
      final cityMatch = driver.cityName?.toLowerCase().contains(_searchTerm) ?? false;
