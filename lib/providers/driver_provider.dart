...
import 'package:lecotour_dashboard/models/driver_car.dart';
import 'package:supabase_flutter/supabase_flutter.dart';

class DriverProviderState {
  final List<Driver> drivers;
  final List<Car> cars;
  final List<DriverCar> driverCarRelationships;
  ...
  DriverProviderState({
    this.drivers = const [],
    this.cars = const [],
    this.driverCarRelationships = const [],
  ...
  DriverProviderState copyWith({
    List<Driver>? drivers,
    List<Car>? cars,
    List<DriverCar>? driverCarRelationships,
    bool? isLoading,
    String? errorMessage,
    ...
    return DriverProviderState(
      ...
      cars: cars ?? this.cars,
      driverCarRelationships: driverCarRelationships ?? this.driverCarRelationships,
  ...
  List<Car> getCarsForDriver(int driverId) {
    // Buscar carros atribuídos ao motorista através da tabela driver_car
    final driverCarIds = driverCarRelationships
        .where((dc) => dc.driverId == driverId && dc.isActive)
        .map((dc) => dc.carId)
        .toList();
    
    return cars.where((car) => driverCarIds.contains(car.id)).toList();
  }

  List<Car> getAvailableCars() {
    // Buscar carros que não estão atribuídos a nenhum motorista
    final assignedCarIds = driverCarRelationships
  ...
  List<Car> get availableCars => getAvailableCars();

  List<Car> getAssignedCars() {
    // Buscar todos os carros que estão atribuídos a algum motorista
    final assignedCarIds = driverCarRelationships
        .where((dc) => dc.isActive)
        .map((dc) => dc.carId)
        .toSet();
...
class DriverNotifier extends StateNotifier<DriverProviderState> {
  ...
  Future<void> fetchInitialData() async {
    print('DriverProvider: fetchInitialData called');
    state = state.copyWith(isLoading: true, clearError: true);
    try {
      print('DriverProvider: Making database queries');
      final driverResponseFuture =
          _supabase.from('driver').select().order('name', ascending: true);
      final carResponseFuture = _supabase
          .from('car')
          .select()
          .order('make', ascending: true)
          .order('model', ascending: true);
      final driverCarResponseFuture = _supabase
          .from('driver_car')
          .select()
          .eq('is_active', true);
      ...
      final responses = await Future.wait([
        ...
        carResponseFuture,
        driverCarResponseFuture,
      ]);

      print('DriverProvider: Database queries completed');
      final driverData = responses[0] as List<dynamic>;
      final carData = responses[1] as List<dynamic>;
      final driverCarData = responses[2] as List<dynamic>;

      print('DriverProvider: Found ${driverData.length} drivers, ${carData.length} cars, and ${driverCarData.length} driver-car relationships');
      ...
      final cars = carData
          ...
          .toList();
      final driverCarRelationships = driverCarData
      ...
      state = state.copyWith(
        drivers: drivers,
        cars: cars,
        driverCarRelationships: driverCarRelationships,
        isLoading: false,
      );
      print('DriverProvider: State updated successfully');
    } catch (e) {
      print('DriverProvider: Error fetching data: $e');
      state = state.copyWith(
          isLoading: false, errorMessage: 'Erro ao buscar dados: $e');
    }
  }
  ...
  Future<bool> addDriver({
    ...
    try {
      ...
      if (carIds != null && carIds.isNotEmpty) {
        await _assignCarsToDriver(newDriver.id, carIds);
      }
  ...
  Future<bool> updateDriver({
    ...
    try {
      ...
      if (carIds != null) {
        await _updateDriverCars(driverId, carIds);
      }
  ...
  Future<void> _assignCarsToDriver(int driverId, List<int> carIds) async {
  ...
  Future<void> _updateDriverCars(int driverId, List<int> newCarIds) async {
    ...
    if (newCarIds.isNotEmpty) {
      await _assignCarsToDriver(driverId, newCarIds);
    }
  }

  Future<bool> assignCarToDriver(int driverId, int carId) async {
    try {
      await _supabase.from('driver_car').insert({
        'driver_id': driverId,
        'car_id': carId,
