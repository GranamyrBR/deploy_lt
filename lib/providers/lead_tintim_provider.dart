...
class LeadTintimNotifier extends StateNotifier<GroupedLeadsState> {
  ...
  bool needsRefresh(Duration ttl) {
    ...
    return state.groupedLeads.isEmpty ||
        last == null ||
        DateTime.now().difference(last) > ttl;
  }
  ...
  Future<void> fetchMoreLeads() async {
    print(
        'ℹ️ LeadTintimNotifier: fetchMoreLeads() chamado, mas todos os dados já foram carregados');
  ...
  Future<void> _fetchLeads() async {
    try {
      ...
      final response = await _supabase
          ...
          .order('datelast',
              ascending: false); // Depois por data dentro de cada telefone
      ...
      print(
          '✅ LeadTintimNotifier: Dados brutos recebidos (TODOS os registros): ${leadsData.length} itens');

      if (queryTime > 1000) {
        print(
            '⚠️  LeadTintimNotifier: PERFORMANCE LENTA - Consulta demorou ${queryTime}ms');
      }
      ...
      print(
          '🔍 LeadTintimNotifier: Processando ${leadsData.length} registros...');
      ...
      final totalLeads = allGroupedLeads.values
          .fold<int>(0, (sum, leads) => sum + leads.length);

      print(
          '📊 LeadTintimNotifier: TODOS os leads agrupados por telefone: $totalGroups grupos');
      allGroupedLeads.forEach((phone, leads) {
        print(
            '📱 LeadTintimNotifier: $phone: ${leads.length} mensagens (HISTÓRICO COMPLETO)');
      });
      ...
      print(
          '✅ LeadTintimNotifier: Estado atualizado com $totalGroups grupos de leads (HISTÓRICO COMPLETO)');
      print(
          '✅ LeadTintimNotifier: Total de registros carregados: ${leadsData.length}');
