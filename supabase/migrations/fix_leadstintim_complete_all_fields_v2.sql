...
-- V2: Corrigido parsing de datas com "às" no formato
-- ============================================
...
SET
    ...
    datefirst = COALESCE(
        datefirst, 
        TO_TIMESTAMP(
            REPLACE((body::jsonb)->'lead'->>'created', ' às ', ' '),
    ...
    datelast = COALESCE(
        datelast, 
        TO_TIMESTAMP(
            REPLACE((body::jsonb)->'lead'->>'updated', ' às ', ' '),
            'YYYY-MM-DD HH24:MI:SS'
        )
    )
WHERE 
