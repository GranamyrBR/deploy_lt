-- 🔄 Script de limpeza CORRIGIDO - versão com FK por telefone
...
SELECT 
    'sale.customer_id type: ' || data_type || '(' || udt_name || ')' as info
FROM information_schema.columns 
WHERE table_schema = 'public' AND table_name = 'sale' AND column_name = 'customer_id';

SELECT 
    'contact.phone type: ' || data_type || '(' || udt_name || ')' as info  
FROM information_schema.columns 
WHERE table_schema = 'public' AND table_name = 'contact' AND column_name = 'phone';
...
BEGIN
    ...
    -- Obter ou criar contato padrão (usando telefone como identificador)
    SELECT phone INTO default_contact_phone FROM public.contact WHERE phone = '+5511999999999' LIMIT 1;
    
    IF default_contact_phone IS NULL THEN
        ...
        BEGIN
            ...
            RETURNING phone INTO default_contact_phone;
            RAISE NOTICE 'Contato padrão criado com telefone: %', default_contact_phone;
        EXCEPTION
            WHEN OTHERS THEN
                -- Se houver algum erro, usar o primeiro contato existente
                SELECT phone INTO default_contact_phone FROM public.contact LIMIT 1;
                IF default_contact_phone IS NULL THEN
    ...
    IF default_contact_phone IS NOT NULL THEN
        -- Corrigir vendas sem cliente (usando telefone como FK)
        UPDATE public.sale 
        SET customer_id = (SELECT id FROM public.contact WHERE phone = default_contact_phone LIMIT 1)
        WHERE customer_id IS NULL;
