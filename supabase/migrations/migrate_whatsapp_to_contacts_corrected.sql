-- =====================================================
-- MIGRAÇÃO CORRIGIDA DE LEADS DO WHATSAPP PARA TABELA CONTACT
-- =====================================================
-- Este script migra leads únicos (por telefone) da tabela leadstintim 
-- para a tabela contact, preservando os nomes reais e evitando duplicatas

-- Primeiro, vamos verificar quantos leads únicos temos no WhatsApp
...
SELECT 'Telefones que já existem na tabela contact:' as info;
...
-- Função para extrair código do país do telefone
-- Assume formato +55 para Brasil, +1 para EUA, etc.
...
BEGIN
  ...
  ELSIF phone_number LIKE '+39%' THEN
    RETURN 'Itália';
  ELSE
    -- Se não conseguir identificar, assume Brasil como padrão
    RETURN 'Brasil';
  END IF;
END;
$$ LANGUAGE plpgsql;

-- Função para extrair estado do telefone (Brasil e EUA)
...
-- Verificar source WhatsApp (id = 13)
SELECT 'VERIFICANDO SOURCE WHATSAPP:' as info;
INSERT INTO source (id, name, description, created_at, updated_at)
SELECT 13, 'WhatsApp', 'Contatos migrados do WhatsApp (leadstintim)', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM source WHERE id = 13);

SELECT id, name, description FROM source WHERE id = 13;

-- MIGRAÇÃO CORRIGIDA: Inserir/atualizar leads usando UPSERT com telefone como chave primária
-- PRESERVA OS NOMES REAIS da tabela leadstintim
SELECT 'INICIANDO MIGRAÇÃO CORRIGIDA:' as info;
...
WHERE l.phone IS NOT NULL 
  AND l.phone != '' 
  AND TRIM(l.phone) != ''
  AND LENGTH(TRIM(l.phone)) >= 10 -- Telefones válidos
...
-- Mostrar estatísticas dos nomes migrados
SELECT 'ESTATÍSTICAS DOS NOMES:' as info;
SELECT 
  CASE 
...
-- Mostrar alguns exemplos dos contatos migrados
SELECT 'EXEMPLOS DE CONTATOS MIGRADOS:' as info;
SELECT 
  id,
  name,
  phone,
...
WHERE source_id = 13
ORDER BY 
  CASE WHEN name != 'Contato WhatsApp' THEN 0 ELSE 1 END,
  created_at DESC
LIMIT 15;

-- Verificar distribuição por país
SELECT 'DISTRIBUIÇÃO POR PAÍS:' as info;
SELECT 
  country,
  COUNT(*) as total,
  ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 2) as percentual
...
SELECT 'DISTRIBUIÇÃO POR ESTADO (BRASIL):' as info;
SELECT 
  state,
  COUNT(*) as total,
  ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 2) as percentual
...
SELECT 'QUALIDADE DOS DADOS MIGRADOS:' as info;
SELECT 
  'Total de contatos migrados' as metrica,
  COUNT(*) as valor
FROM contact 
WHERE source_id = 13
...
SELECT 
  'Contatos com nomes reais' as metrica,
  COUNT(*) as valor
FROM contact 
WHERE source_id = 13 
...
SELECT 
  'Contatos com país identificado' as metrica,
  COUNT(*) as valor
FROM contact 
WHERE source_id = 13 
...
SELECT 
  'Contatos com estado identificado' as metrica,
  COUNT(*) as valor
FROM contact 
WHERE source_id = 13 
  AND state IS NOT NULL;

SELECT 'MIGRAÇÃO CONCLUÍDA COM SUCESSO!' as resultado_final;

-- Limpar funções temporárias (opcional)
-- DROP FUNCTION IF EXISTS get_country_from_phone(TEXT);
-- DROP FUNCTION IF EXISTS get_state_from_phone(TEXT);

-- =====================================================
-- MELHORIAS IMPLEMENTADAS:
-- 1. Preservação dos nomes reais da tabela leadstintim
-- 2. Validação de comprimento mínimo para nomes (>= 2 caracteres)
-- 3. Validação de telefones válidos (>= 10 dígitos)
-- 4. Estatísticas detalhadas da qualidade da migração
-- 5. Verificação automática do source WhatsApp
-- =====================================================