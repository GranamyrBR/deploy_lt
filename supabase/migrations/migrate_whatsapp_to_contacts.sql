-- =====================================================
-- MIGRAÇÃO DE LEADS DO WHATSAPP PARA TABELA CONTACT
-- =====================================================
-- Este script migra leads únicos (por telefone) da tabela leadstintim 
-- para a tabela contact, evitando duplicatas

-- Primeiro, vamos verificar quantos leads únicos temos no WhatsApp
SELECT 'Leads únicos do WhatsApp por telefone:' as info;
...
-- Inserir leads únicos do WhatsApp na tabela contact
-- Evita duplicatas verificando se o telefone já existe
INSERT INTO contact (
  name,
  phone,
  country,
...
SELECT DISTINCT ON (l.phone)
  CASE 
    WHEN l.name IS NOT NULL AND TRIM(l.name) != '' THEN TRIM(l.name)
    ELSE 'Contato WhatsApp'
  END as name,
  l.phone,
  get_country_from_phone(l.phone) as country,
  get_state_from_phone(l.phone) as state,
  13 as source_id, -- WhatsApp (conforme source_rows.csv)
  12 as contact_category_id, -- Lead (conforme monday_screen.dart)
...
SELECT 'Contatos migrados do WhatsApp:' as info;
